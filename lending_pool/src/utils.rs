    ...
    + price_aggregator_proxy::PriceAggregatorModule
{
    ...
    fn get_collateral_available_for_token(&self, account_position: u64, token_id: TokenIdentifier) -> BigUint {
    ...
    fn send_amount_in_dollars_to_liquidator(
        ...
        for mut dp in deposit_position_iter {
            ...
            if amount_in_dollars_available_for_this_bp <= amount_to_send {
                ...
                let partial_amount_to_send = (&amount_to_send * BP / &dp_data.price) / BP;
                self.send()
    ...
    //     let borrow_position_iter = borrow_positions
    //         .iter()
    //         .filter(|bp| bp.owner_nonce == account_position && bp.token_id == token_id);
